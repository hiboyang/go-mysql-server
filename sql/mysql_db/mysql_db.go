...
func (db *MySQLDb) UserActivePrivilegeSet(ctx *sql.Context) PrivilegeSet {
	if db.updateCounter == ctx.Session.GetPrivilegeSetCounter() {
		// If the counters are equal, we can guarantee that the privilege set exists and is valid
		return ctx.Session.GetPrivilegeSet().(PrivilegeSet)
	}
	...
	ctx.Session.SetPrivilegeSet(db.updateCounter, privSet)
	return privSet
}
